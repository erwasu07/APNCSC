    ...
    <div className="bg-white dark:bg-slate-900/40 border-b border-slate-200/50 dark:border-slate-800/60 py-10 transition-colors duration-300 scroll-mt-24" id="services">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-1.5 mb-8">
          ...
          >
            All Categories ({SERVICES_LIST.length})
          </button>
          {categoriesKeys.map(key => {
            ...
            return (
              ...
              >
                <span className={`h-1.5 w-1.5 rounded-full ${theme.text} bg-current`}></span>
        ...
        <div className="flex flex-row overflow-x-auto snap-x snap-mandatory gap-4 pb-6 lg:grid lg:grid-cols-2 xl:grid-cols-4 lg:gap-8 lg:overflow-x-visible w-full scrollbar-thin">
          {categoriesKeys.map(cat => {
            ...
            return (
              ...
              >
                {/* Category Header */}
                <div className={`bg-gradient-to-r ${theme.gradient} text-white px-2 py-2.5 sm:px-5 sm:py-4 flex items-center justify-between`}>
                  <h3 className="font-extrabold tracking-tight text-[10px] sm:text-xs md:text-sm uppercase font-display truncate">
                    {CATEGORY_LABELS[cat]}
                  </h3>
                  <span className="hidden sm:inline-block bg-white/20 text-white text-[10px] font-bold px-2 py-0.5 rounded-md font-mono">
                    {list.length} Items
                  </span>
                </div>
          ...
          >
            ...
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 relative bg-slate-50 dark:bg-slate-950/60 flex items-start justify-between">
              <div className="space-y-1 pr-8">
                <span className="px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
